...
function setup() {
  ...
  const formatChangeLog = (changelog) => {
    return changelog
      ...
      .replace(/^[-*] /mg, '')
      .replace(/#(\d+)$/gm, `<a target="_blank" href="https://github.com/MeshPager/fw/pull/$1">#$1</a>`)
//      .split(/\r?\n/)
//      .map(l => `* ${l}`)
//      .join('\n')
  }
  ...
  const openSerialGUI = () => {
    window.open('https://config.meshcore.dev','meshpager_config','directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no,width=1000,height=800');
  }
  ...
  const openSerialCon = async() => {
    ...
    serialCon.content =  '-------------------------------------------------------------------------\n';
    serialCon.content += 'Welcome to MeshPager serial console.\n'
  ...
  const flashDevice = async() => {
    ...
    if(!flashFiles[0]) {
      alert('Cannot find configuration for flash file! please report this to GitHub Issues.')
      flasherCleanup();
      return;
    }
